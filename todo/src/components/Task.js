...
// Quick status update without other fields
const updateTaskStatus = async(newStatus) => {
    try {
        console.log('updating just the status to:', newStatus)
...
const updateTask = async() => {
    try {
        console.log('proceeding to update all task details')
        const response = await axios.patch('http://localhost:5000/api/edittask', {
            taskId: taskId,
            taskName: taskName,
            ...
            <div className={styles.showTaskContainer} style={{display: taskToggle? "block" : "none"}}>
                <div className={styles.exitButtonContainer}>
                    ...
                    <div className={styles.editDeleteContainer}>
                        <svg className={styles.editButton} onClick={()=> {
                        ...
                        <svg className={styles.deleteButton} onClick={()=> {
                            onDelete()
                            closeTask()
                        }} xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000"><path d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z"/></svg>
                    </div>
                </div>
                {editState ? (
                    <div className={styles.taskInfo}>
                        <h1 className={styles.taskHeader}>Task</h1>
                        <input className={styles.taskNameInput} type='text' onChange={(e)=> setTaskName(e.target.value)} value={taskName}></input>
                        <textarea className={styles.taskDescriptionInput} type='text' onChange={(e)=> setTaskDescription(e.target.value)} value={taskDescription}></textarea>
                        <select 
